...
import { ExceptionsWrapper, GuardWrapper, IServiceHandlerVoidAsync, Result, ResultError, ResultFactory, sealed, Service, VOID_RESULT, VoidResult } from "@kishornaik/utils";
...
export class DemoProcessService implements IDemoRecordProcessService {
  public async handleAsync(params: DemoRecord): Promise<Result<VoidResult, ResultError>> {
    return await ExceptionsWrapper.tryCatchResultAsync(async ()=>{
      ...
      if(guard.isErr())
        return ResultFactory.errorInstance(guard.error);
